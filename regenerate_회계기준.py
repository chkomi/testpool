...
회계기준_answers = answer_data.get('회계기준', {})
...
# Filter for 회계기준 explanations
회계기준_explanations = {}
for item in explanation_data:
    if item.get('과목') == '회계기준':
        회계기준_explanations[str(item['문항'])] = {
            'explanation': item['해설'],
            'url': item.get('url', '')
        }

# Parse original text
with open('/Users/hyungchangyoun/Documents/project/testpool/2025/8. 회계기준.txt', 'r', encoding='utf-8') as f:
    content = f.read()
...
for i, question in enumerate(questions):
    ...
    # Apply answer
    if str(question_num) in 회계기준_answers:
        answer_value = 회계기준_answers[str(question_num)]
        question['correct'] = answer_map.get(answer_value, 'a')
    
    # Apply explanation  
    if str(question_num) in 회계기준_explanations:
        exp_data = 회계기준_explanations[str(question_num)]
        question['explanation'] = exp_data['explanation']
        question['urls'] = [exp_data['url']] if exp_data['url'] else [f"https://www.law.go.kr/학칙공단/한국농어촌공사 회계기준/(9999,20241231)/관련조항"]
    else:
        question['explanation'] = "회계기준 관련 조항"
        question['urls'] = [f"https://www.law.go.kr/학칙공단/한국농어촌공사 회계기준/(9999,20241231)/관련조항"]
...
# Write new file
with open('/Users/hyungchangyoun/Documents/project/testpool/2025-data-회계기준.js', 'w', encoding='utf-8') as f:
    f.write(js_content)

print(f"✅ 회계기준 데이터 재생성 완료: {len(questions)}개 문제")