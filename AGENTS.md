...
## 2024 시험 파일(YYYY-exam.js) 정답/해설 관리 지침
- 소스 우선순위:
  - 정답(문자): `2024/` 폴더의 “정답 및 해설.pdf”를 최우선으로 따릅니다(공식정답 기준 유지).
  - 해설(근거): `2025/법령집` 텍스트((법령) *.txt)를 인용해 근거를 제시합니다.
- 불일치 처리:
  - 공식정답과 최신 법령 해석이 어긋나는 경우라도 ‘correct’ 값은 공식정답을 유지합니다.
  - 해설에는 “공식 정답 기준”임을 명시하고, 해당 조문(항/호 포함)과 문언 차이를 간략히 설명합니다.
- 편집 원칙(2024-exam.js):
  - 문제 텍스트/선택지는 변경하지 않습니다(오탈자 제외). ‘correct’와 ‘explanation’만 수정.
  - 해설 형식은 2025 지침과 동일하게 유지합니다(문제 N — 【정답】 x — 【해설】, 관련 법령/정답 이유/오답 분석).
  - 가능하면 `urls` 또는 `url` 필드에 근거 링크를 추가하여 참조 버튼이 작동하도록 합니다.
- 검증 체크리스트(2024):
  - ‘correct’가 2024 공식 정답과 일치하는지 수동 대조(현재 자동 매핑 파일 없음).
  - 해설의 조문 표기(법/시행령, 조·항·호)가 정확한지, 최신 개정 기준인지 `2025/법령집`으로 재확인.
- 커밋 규칙(2024 관련):
  - 메시지: `fix(2024-exam): QNN answer/explanation …` 또는 `docs(2024-exam): refine explanation QNN`.
  - 본문에 인용 근거(예: (법령) 파일명·조문) 간단 표기.
  - 변경 범위 최소화(라인 단위 diff 축소).
