...
def parse_employment_rules_fixed():
    """5. 취업규칙.txt 파일을 정확히 파싱해서 47문제를 만듭니다."""
    ...
    for num in unique_numbers:
        ...
        for line in lines:
            ...
            if option_match:
                ...
                # 보기 이후는 문제 텍스트의 연속 (보기가 아닌 경우)
                if not re.match(r'^[㉠-㉥]', line) and not re.match(r'^[가-하]', line):
                    question_text += line + " "
        ...
        if len(options) == 4:
            ...
            print(f"문제 {num}: 보기 수가 4개가 아님 ({len(options)}개) - {options}")
...
def save_questions_to_js(questions):
    ...
    with open('취업규칙_47_questions_fixed.js', 'w', encoding='utf-8') as f:
        f.write(js_content)
    
    print(f"취업규칙_47_questions_fixed.js 파일이 생성되었습니다. (총 {len(questions)}문제)")

if __name__ == "__main__":
    questions = parse_employment_rules_fixed()
